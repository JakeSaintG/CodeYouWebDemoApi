...
export class ContactRequestsTable {
    private dbContext!: sqlite.Database;
    ...
    setDbContext = () => {
        this.dbContext = new sqlite(this.fileLocation);
    };

    insertNewContactRequest = (contactReq: ContactRequest) => {
    ...
    selectAllContactRequests = (): ContactRequest[] => {
        // Select all contact requests from database.
        const selectAllContactRequests: unknown[] = this.dbContext
    ...
    createAndPopulateContactsTable = (contactRequests: ContactRequest[]) => {
        /*
            Create table:
